...
docker build -t predecoder-train --build-arg TORCH_CUDA=cu126 .
...
| `TORCH_CUDA` | `cu121` | PyTorch CUDA wheel tag (e.g. `cu121`, `cu126`, `cu130`). |
...
2. **CUDA version**: set `TORCH_CUDA=cuXXX` to match your driver
   (e.g. `cu121` for CUDA 12.1, `cu126` for CUDA 12.6).
