...
def compute_ler_for_p_range(model, device, dist, cfg, p_values, distance, rank=0, n_rounds=None):
    ...
    results = {
        'p_values': p_values,
        'X': {'ler': [], 'ler_err': [], 'pymatch_ler': [], 'pymatch_ler_err': []},
        'Z': {'ler': [], 'ler_err': [], 'pymatch_ler': [], 'pymatch_ler_err': []},
    }
    ...
    for p in p_values:
        ...
        try:
            ...
            if isinstance(result, dict) and 'X' in result and 'Z' in result:
                ...
                if all(v is not None for v in [x_ler, x_ler_err, z_ler, z_ler_err, 
                                                x_pymatch_ler, x_pymatch_ler_err, 
                    ...
                    if rank == 0:
                        ...
                        print(f"  {'PyMatching latency - X basis (µs/round):':<{label_w}}{x_lat_base_f:>15.3f}  {x_lat_post_f:>17.3f}")
                        print(f"  {'PyMatching latency - Z basis (µs/round):':<{label_w}}{z_lat_base_f:>15.3f}  {z_lat_post_f:>17.3f}")
                        print(f"  {'PyMatching latency - Avg (µs/round):':<{label_w}}{avg_lat_base:>15.3f}  {avg_lat_post:>17.3f}")

                        # LER (unitless)
                        avg_ler_base = _avg(x_pymatch_ler, z_pymatch_ler)
                        avg_ler_post = _avg(x_ler, z_ler)
                        print(f"  {'LER - X basis:':<{label_w}}{float(x_pymatch_ler):>15.6f}  {float(x_ler):>17.6f}")
                        print(f"  {'LER - Z basis:':<{label_w}}{float(z_pymatch_ler):>15.6f}  {float(z_ler):>17.6f}")
                        print(f"  {'LER - Avg:':<{label_w}}{avg_ler_base:>15.6f}  {avg_ler_post:>17.6f}")
                else:
                    if rank == 0:
...
def create_threshold_plot(all_results, distances, output_path, rank=0):
    ...
    for i, distance in enumerate(distances):
        ...
        if np.any(valid_x):
            ax1.plot(p_values[valid_x], x_ratio[valid_x],
                    marker='o', markersize=10, linewidth=2,
                    label=f'd={distance} (LER)', color=color, alpha=0.8)
    ...
    for i, distance in enumerate(distances):
        ...
        if np.any(valid_z):
            ax2.plot(p_values[valid_z], z_ratio[valid_z],
                    marker='o', markersize=10, linewidth=2,
                    label=f'd={distance} (LER)', color=color, alpha=0.8)
    ...
    for i, distance in enumerate(distances):
        ...
        if np.any(valid_x):
            ax1_abs.errorbar(p_values[valid_x], x_ler[valid_x], yerr=x_ler_err[valid_x],
                            marker='o', markersize=7, linewidth=2, capsize=6, capthick=2,
                            elinewidth=2, label=f'd={distance} (Model)', color=color, alpha=0.8)
        
        # Plot PyMatching-only baseline (dashed line, no error bars)
        valid_pm = ~np.isnan(x_pymatch_ler)
        if np.any(valid_pm):
            ax1_abs.plot(p_values[valid_pm], x_pymatch_ler[valid_pm],
                        linestyle='--', linewidth=2, marker='x', markersize=5,
                        label=f'd={distance} (PyMatch)', color=color, alpha=0.6)
    ...
    for i, distance in enumerate(distances):
        ...
        if np.any(valid_z):
            ax2_abs.errorbar(p_values[valid_z], z_ler[valid_z], yerr=z_ler_err[valid_z],
                            marker='o', markersize=7, linewidth=2, capsize=6, capthick=2,
                            elinewidth=2, label=f'd={distance} (Model)', color=color, alpha=0.8)
        ...
        if np.any(valid_pm):
            ax2_abs.plot(p_values[valid_pm], z_pymatch_ler[valid_pm],
                        linestyle='--', linewidth=2, marker='x', markersize=5,
                        label=f'd={distance} (PyMatch)', color=color, alpha=0.6)
...
def run_threshold_plot(model, device, dist, cfg):
    ...
    if verbose and rank == 0:
        ...
        print(f"  Correlated matching: {'ENABLED (two-pass, ~2.7x slower)' if enable_correlated else 'DISABLED (standard, faster)'}")
    ...
    if is_public_cfg:
        ...
        if hasattr(cfg, 'threshold') and hasattr(cfg.threshold, 'n_rounds') and cfg.threshold.n_rounds is not None:
    ...
    for i, distance in enumerate(distances):
        ...
        results = compute_ler_for_p_range(model, device, dist, cfg, p_values, distance, rank, n_rounds=n_rounds)
